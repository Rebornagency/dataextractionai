...
#################################################
# GPT Data Extractor Module (Unchanged from previous fix)
#################################################
    ...
    def _extract_with_gpt(self, prompt: str) -> Optional[Dict[str, Any]]:
        try:
            ...
            try:
                ...
                logger.error(f"JSON parse error: {e}"); json_match = re.search(r'\{.*\}', cleaned_response_text, re.DOTALL)
                if json_match:
                    ...
                    except json.JSONDecodeError: logger.error("Regex parse error.")
                else: logger.error("No JSON found.")
                return None
