"""
Data Extraction AI - Combined Module (Enhanced + App Fees + NumPy Fix + Syntax Fixes + Debug Logging)
...
class FilePreprocessor:
    ...
    def _process_pdf(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"DEBUG: Final combined_text length (PDF): {len(result.get('combined_text', ''))}") # DEBUG LOG
        return result

    def _process_excel(self, file_path: str) -> Dict[str, Any]:
        """Process Excel files using pandas."""
        logger.debug(f"DEBUG: Starting _process_excel for: {os.path.basename(file_path)}") # DEBUG LOG
        ...
        try:
            ...
            logger.debug(f"DEBUG: Excel sheets found: {sheet_names}") # DEBUG LOG

            for sheet_name in sheet_names:
                logger.debug(f"DEBUG: Processing sheet '{sheet_name}'") # DEBUG LOG
                try:
                    ...
                    if df.empty:
                         logger.debug(f"DEBUG: Sheet '{sheet_name}' is empty after cleaning, skipping.") # DEBUG LOG
                         continue

                    header_row_index = self._find_header_row(df)
                    if header_row_index is not None:
                         logger.debug(f"DEBUG: Detected header row {header_row_index} in sheet '{sheet_name}'.") # DEBUG LOG
                    ...
                    logger.debug(f"DEBUG: Added text representation for sheet '{sheet_name}'. Length: {len(text_rep)}") # DEBUG LOG
            ...
            logger.debug(f"DEBUG: Final combined_text length (Excel): {len(result.get('combined_text', ''))}") # DEBUG LOG
        return result

    def _process_csv(self, file_path: str) -> Dict[str, Any]:
        """Process CSV files using pandas."""
        logger.debug(f"DEBUG: Starting _process_csv for: {os.path.basename(file_path)}") # DEBUG LOG
        result = {'data': [], 'text_representation': "", 'combined_text': ""}
        text_rep = ""

        try:
            encoding = self._detect_encoding(file_path)
            logger.debug(f"DEBUG: Reading CSV with encoding: {encoding}") # DEBUG LOG
            ...
            logger.debug(f"DEBUG: Final combined_text length (CSV): {len(result.get('combined_text', ''))}") # DEBUG LOG
        return result

    def _process_txt(self, file_path: str) -> Dict[str, Any]:
        """Process TXT files."""
        logger.debug(f"DEBUG: Starting _process_txt for: {os.path.basename(file_path)}") # DEBUG LOG
        result = {'text': "", 'combined_text': ""}
        cleaned_text = ""
        try:
            encoding = self._detect_encoding(file_path)
            logger.debug(f"DEBUG: Reading TXT with encoding: {encoding}") # DEBUG LOG
            ...
            logger.debug(f"DEBUG: Final combined_text length (TXT): {len(result.get('combined_text', ''))}") # DEBUG LOG
        return result

    # _detect_encoding, _clean_text, _is_header_row, _find_header_row, _score_header_row remain the same
...
class DocumentClassifier:
    ...
    def _extract_period_from_content(self, text_or_data: Union[str, Dict[str, Any]]) -> Optional[str]:
        ...
        months_full=['January','February','March','April','May','June','July','August','September','October','November','December']; months_abbr=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']; months_pattern='|'.join(months_full+months_abbr); year_pattern=r'(20\d{2})'
        pattern1=rf'(?:For the (?:Month|Period)\s+(?:Ended|Ending)\s+)?({months_pattern})\s+(\d{{1,2}})[\s,]+{year_pattern}'; match1=re.search(pattern1,text,re.IGNORECASE)
        if match1: month_full=self._standardize_month(match1.group(1),months_full,months_abbr); logger.debug(f"Period pattern 1: {month_full} {match1.group(3)}"); return f"{month_full} {match1.group(3)}"
        pattern2=rf'({months_pattern})[\s.,_-]+{year_pattern}'; match2=re.search(pattern2,text,re.IGNORECASE)
        if match2: month_full=self._standardize_month(match2.group(1),months_full,months_abbr); logger.debug(f"Period pattern 2: {month_full} {match2.group(2)}"); return f"{month_full} {match2.group(2)}"
        quarter_words=['First','Second','Third','Fourth']; pattern3=rf'(?:(Q[1-4])|({"|".join(quarter_words)})\s+Quarter)[\s.,_-]*{year_pattern}|(?:Quarter\s+(?:Ended|Ending)\s+(?:{months_pattern})\s+\d{{1,2}}[\s.,_-]*)({year_pattern})'; match3=re.search(pattern3,text,re.IGNORECASE)
        if match3: year_str=match3.group(3) or match3.group(4);
        if match3.group(1): period_str=f"{match3.group(1)} {year_str}"
        elif match3.group(2): period_str=f"{match3.group(2)} Quarter {year_str}"
        else: period_str=f"Quarter in {year_str}"; logger.debug(f"Period pattern 3: {period_str}"); return period_str
        pattern4=rf'(?:For the Year\s+(?:Ended|Ending)|FY|Fiscal Year|Calendar Year)[\s.,_-]*({year_pattern})'; match4=re.search(pattern4,text,re.IGNORECASE)
        if match4: period_str=f"Year {match4.group(1)}"; logger.debug(f"Period pattern 4: {period_str}"); return period_str
        for match5 in re.finditer(rf'\b({year_pattern})\b',text): year_str=match5.group(1); context_window=text[max(0,match5.start()-40):min(len(text),match5.end()+40)];
        if any(kw in context_window.lower() for kw in ['period','month','year','date','quarter','budget','actual','statement','report','ended','ending']):
             ...
             if month_nearby_match: month_full=self._standardize_month(month_nearby_match.group(1),months_full,months_abbr); period_str=f"{month_full} {year_str}"; logger.debug(f"Period pattern 5 (Y+M Context): {period_str}"); return period_str
             else: period_str=f"Year {year_str}"; logger.debug(f"Period pattern 5 (Y Context): {period_str}"); return period_str
        logger.warning("Could not extract period from content."); return None
    ...
    def _rule_based_classification(self, text: str) -> Dict[str, Any]:
        ...
        if not text: return result; text_lower=text.lower(); doc_type='Unknown'; type_confidence=0.0; budget_score=text_lower.count('budget')+text_lower.count('variance'); actual_score=text_lower.count('actual')+text_lower.count('income statement')+text_lower.count('profit and loss'); prior_year_score=text_lower.count('prior year')+text_lower.count('previous year');
        if budget_score>actual_score and budget_score>0: doc_type='Budget'; type_confidence=min(0.8+budget_score*0.1,1.0)
        elif actual_score>0:
            if prior_year_score>0: doc_type='Prior Year Actual'; type_confidence=min(0.8+prior_year_score*0.1,1.0)
            else: doc_type='Actual Income Statement'; type_confidence=min(0.7+actual_score*0.05,0.9);
        ...
        if period:
            ...
            current_confidence = min(current_confidence + 0.15, 1.0); # Combine confidence
    ...
    def _gpt_classification(self, text: str) -> Dict[str, Any]:
        ...
        if not text_sample: logger.warning("Empty text for GPT classification."); return default_result; prompt=f"""Analyze the beginning of this financial document to determine its type and the period it covers...\nRespond ONLY with a JSON object containing 'document_type' and 'period' fields. Example: {{"document_type": "Actual Income Statement", "period": "March 2025"}}""" # Simplified
        try:
            response=self.client.chat.completions.create(model="gpt-4",messages=[{"role":"system","content":"You are an expert financial document classifier... Respond only JSON."},{"role":"user","content":prompt}],temperature=0.1,max_tokens=100); response_text=response.choices[0].message.content.strip(); logger.info(f"GPT classification raw: {response_text}");
            try:
                cleaned_response_text=response_text.strip('`').strip();
                if cleaned_response_text.startswith('json'): cleaned_response_text=cleaned_response_text[4:].strip(); result=json.loads(cleaned_response_text);
                if 'document_type' in result and 'period' in result:
                    if result['document_type'] not in self.document_types: result['document_type']='Unknown'; logger.info(f"Parsed GPT classification: {result}"); return result
                else: raise json.JSONDecodeError("Missing fields",cleaned_response_text,0)
            except json.JSONDecodeError as e:
                logger.error(f"Error parsing JSON from GPT classification: {e}"); doc_type_match=re.search(r'"document_type":\s*"([^"]+)"',response_text); period_match=re.search(r'"period":\s*"([^"]*)"',response_text); doc_type_res=doc_type_match.group(1) if doc_type_match else 'Unknown'; period_res=period_match.group(1) if period_match else None;
                if doc_type_res not in self.document_types: doc_type_res='Unknown'; logger.warning(f"Falling back to regex for classification: Type='{doc_type_res}', Period='{period_res}'"); return {'document_type':doc_type_res,'period':period_res}
...
def classify_document(text_or_data: Union[str, Dict[str, Any]], known_document_type: Optional[str] = None) -> Tuple[str, Optional[str]]: result = classifier.classify(text_or_data, known_document_type); return result['document_type'], result['period']
def map_noi_tool_to_extraction_type(noi_tool_type: str) -> str: return classifier.document_type_mapping.get(noi_tool_type, "Unknown")
...
class GPTDataExtractor:
    ...
    def _create_extraction_prompt(self, text: str, document_type: str, period: Optional[str] = None) -> str:
        ...
        prompt = f"""Expert analyst: Extract detailed data from '{document_type}' document{period_context}...\nSample:\n---\n{text_sample}\n---\nExtract precisely (use 0 if missing, verify/calc totals):\n1. PGI: * `gross_potential_rent`... * `loss_to_lease`...\n2. VACANCY: * `physical_vacancy_loss`... * `concessions_free_rent`... * `bad_debt`... * `total_vacancy_credit_loss`...\n3. OTHER INCOME: * `recoveries`... * `parking_income`... * `laundry_income`... * `application_fees`... * `other_misc_income`... * `total_other_income`...\n4. EGI: * `effective_gross_income`...\n5. OPEX: * `property_taxes`... * `insurance`... * `property_management_fees`... * `repairs_maintenance`... * `utilities`... * `payroll`... * `admin_office_costs`... * `marketing_advertising`... * `other_opex`... * `total_operating_expenses`...\n6. NOI: * `net_operating_income`...\n7. RESERVES (Optional): * `replacement_reserves`... * `tenant_improvements`...\nRespond ONLY JSON... Example:\n{{ "document_type": "{document_type}", "period": "{period or 'Unknown'}", "gross_potential_rent": 0, ..., "tenant_improvements": 0 }}""" # Truncated for brevity
        return prompt
    ...
    def _extract_with_gpt(self, prompt: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            start_time = time.time(); response = self.client.chat.completions.create(model="gpt-4", messages=[{"role": "system", "content": "Expert extractor. Respond only JSON."}, {"role": "user", "content": prompt}], temperature=0.1, max_tokens=1500)
            duration = time.time() - start_time; logger.info(f"GPT extraction time: {duration:.2f}s"); response_text = response.choices[0].message.content.strip(); logger.debug(f"GPT raw: {response_text}")
            ...
            try:
                ...
                if isinstance(result, dict): logger.info("Parsed JSON."); return result
                else: logger.error(f"Parsed non-dict: {type(result)}"); return None
            except json.JSONDecodeError as e:
                logger.error(f"Initial JSON parse error: {e}"); json_match = re.search(r'\{.*\}', cleaned_response_text, re.DOTALL)
                if json_match:
                    # Nested try for the regex fallback parsing
                    try: # <--- The try that was missing its except
                        result = json.loads(json_match.group(0));
                        if isinstance(result, dict): logger.warning("Parsed JSON via regex."); return result
                        else: logger.error(f"Regex non-dict: {type(result)}"); return None
                    # --- ADDED THIS EXCEPT BLOCK ---
                    except json.JSONDecodeError as regex_e:
                        logger.error(f"Regex fallback JSON parse error: {regex_e}")
                        # If regex fallback also fails, return None
                        return None
                    # --- END OF ADDED BLOCK ---
                else: logger.error("No JSON object found in the response via regex.")
                return None # Return None if initial parse failed AND regex failed or didn't find JSON

        except (RateLimitError, APIError, APITimeoutError) as api_err: logger.error(f"OpenAI API error: {api_err}"); return None
        except Exception as e: logger.error(f"Unexpected GPT error: {e}", exc_info=True); return None
    # --- END OF FIX ---
    ...
    def _empty_result(self, document_type: str = "Unknown", period: Optional[str] = None) -> Dict[str, Any]:
        return {"document_type": document_type, "period": period or 'Unknown', "gross_potential_rent": 0.0, "loss_to_lease": 0.0, "physical_vacancy_loss": 0.0, "concessions_free_rent": 0.0, "bad_debt": 0.0, "total_vacancy_credit_loss": 0.0, "recoveries": 0.0, "parking_income": 0.0, "laundry_income": 0.0, "application_fees": 0.0, "other_misc_income": 0.0, "total_other_income": 0.0, "effective_gross_income": 0.0, "property_taxes": 0.0, "insurance": 0.0, "property_management_fees": 0.0, "repairs_maintenance": 0.0, "utilities": 0.0, "payroll": 0.0, "admin_office_costs": 0.0, "marketing_advertising": 0.0, "other_opex": 0.0, "total_operating_expenses": 0.0, "net_operating_income": 0.0, "replacement_reserves": 0.0, "tenant_improvements": 0.0}

extractor = GPTDataExtractor()
def extract_financial_data(text_or_data: Union[str, Dict[str, Any]], document_type: str, period: Optional[str] = None) -> Dict[str, Any]: return extractor.extract(text_or_data, document_type, period)

#################################################
# Validation Formatter Module (Unchanged)
#################################################
class ValidationFormatter:
    ...
    def validate_and_format(self, data: Dict[str, Any]) -> Tuple[Dict[str, Any], List[str]]:
        logger.info("Validating/formatting data"); warnings = []; validated_data = data.copy()
        for rule in self.validation_rules:
            try: warnings.extend(rule(validated_data))
            except Exception as e: logger.error(f"Validation rule error {rule.__name__}: {e}", exc_info=True); warnings.append(f"Internal validation error: {rule.__name__}")
        final_output = self._format_output(validated_data)
        if warnings: final_output['validation_warnings'] = warnings; logger.warning(f"Final validation warnings: {warnings}")
    ...
    def _validate_period(self, data: Dict[str, Any]) -> List[str]: warnings = []; period = data.get('period'); if not period or not isinstance(period, str): warnings.append(f"Invalid period: {period}"); data['period'] = 'Unknown'; return warnings
    def _validate_numeric_fields(self, data: Dict[str, Any]) -> List[str]:
        warnings = []
        for field, field_type in self.expected_fields.items():
            is_optional_reserve = field in ['replacement_reserves', 'tenant_improvements']; allowed_types = (float, int) if not is_optional_reserve else (float, int, type(None))
    ...
    def _validate_egi(self, data: Dict[str, Any]) -> List[str]: warnings = []; tolerance = 1.0; gpr = self._clean_value_for_validation(data, 'gross_potential_rent'); vacancy = self._clean_value_for_validation(data, 'total_vacancy_credit_loss'); other_income = self._clean_value_for_validation(data, 'total_other_income'); calculated_egi = gpr - vacancy + other_income; reported_egi = self._clean_value_for_validation(data, 'effective_gross_income'); if abs(calculated_egi - reported_egi) > tolerance: warnings.append(f"Validation Check: EGI ({reported_egi:.2f}) vs calc ({calculated_egi:.2f})."); return warnings
    def _validate_total_opex_sum(self, data: Dict[str, Any]) -> List[str]: warnings = []; tolerance = 1.0; opex_components = ['property_taxes', 'insurance', 'property_management_fees', 'repairs_maintenance', 'utilities', 'payroll', 'admin_office_costs', 'marketing_advertising', 'other_opex']; calculated_sum = sum(self._clean_value_for_validation(data, field) for field in opex_components); reported_total = self._clean_value_for_validation(data, 'total_operating_expenses'); if abs(calculated_sum - reported_total) > tolerance: warnings.append(f"Validation Check: Total OpEx ({reported_total:.2f}) vs sum ({calculated_sum:.2f})."); return warnings
    def _validate_noi_from_egi(self, data: Dict[str, Any]) -> List[str]: warnings = []; tolerance = 1.0; egi = self._clean_value_for_validation(data, 'effective_gross_income'); total_opex = self._clean_value_for_validation(data, 'total_operating_expenses'); calculated_noi = egi - total_opex; reported_noi = self._clean_value_for_validation(data, 'net_operating_income'); if abs(calculated_noi - reported_noi) > tolerance: warnings.append(f"Validation Check: NOI ({reported_noi:.2f}) vs calc ({calculated_noi:.2f})."); return warnings
    def _format_output(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return output
def validate_and_format_data(data: Dict[str, Any]) -> Dict[str, Any]: validator = ValidationFormatter(); formatted_data, _ = validator.validate_and_format(data); return formatted_data
...
app = FastAPI(title="NOI Data Extraction API (v2.2.3)", description="API for extracting detailed financial data", version="2.2.3") # Version bump
...
if "render" in user_agent or "health" in user_agent: logger.info("Skipping auth for health check probe."); return {"status": "healthy", "version": "2.2.3"} # Updated version
...
return {"status": "healthy", "version": "2.2.3"} # Updated version
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting NOI Data Extraction API Server (v2.2.3 - SyntaxFix2) on {host}:{port}...") # Added version note
