...
"""
    ...
    def _validate_extraction_result(self, result: Dict[str, Any], document_type: str) -> None:
        required_fields = [
            'rental_income', 'laundry_income', 'parking_income', 'other_revenue', 'total_revenue',
        ...
        revenue_items = [result.get(k, 0) for k in ['rental_income', 'laundry_income', 'parking_income', 'other_revenue']]
        expense_items = [result.get(k, 0) for k in [
        ...
        if abs(calc_revenue - result.get('total_revenue', 0)) > 1:
            logger.warning(f"Revenue mismatch: calculated={calc_revenue}, reported={result['total_revenue']}")
            result['total_revenue'] = calc_revenue

        if abs(calc_expenses - result.get('total_expenses', 0)) > 1:
            logger.warning(f"Expenses mismatch: calculated={calc_expenses}, reported={result['total_expenses']}")
            result['total_expenses'] = calc_expenses

        if abs(calc_noi - result.get('net_operating_income', 0)) > 1:
            logger.warning(f"NOI mismatch: calculated={calc_noi}, reported={result['net_operating_income']}")
